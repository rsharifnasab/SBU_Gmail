...
public class UserHandler implements Runnable, Serializable {
    ...
    private State state; //current state of game!
    ...
    public UserHandler(Socket socket) {
        ...
        state = State.MAIN_MENU; //Avvalesh too-e MainMenuEm!
        try {
    ...
    private Response produceResponse(Command command) {
        ...
        if (command instanceof CheckLoginValidnessCommand) {
         ...
         else if (command instanceof ChangeStateCommand) {
            returnValue = this.changeStateThings((ChangeStateCommand) command);
          }
        return returnValue;
    }

    private Response changeStateThings(ChangeStateCommand command) {
        this.state = command.getState();
        return new ChangeStateResponse(true);
    }
